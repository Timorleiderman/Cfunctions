...
void cofactor(double src[MATRIX_nXn][MATRIX_nXn], double dst[MATRIX_nXn][MATRIX_nXn], size_t n, double det) {
	...
	for (row = 0; row < n; row++)
		for (col = 0; col < n; col++) {
			...
			cofactorMat[row][col] = pow(-1, (row + col)) * determinant(minerMat, (n - 1));
		}
...
void PrintNxNMatrix(double h[MATRIX_nXn][MATRIX_nXn], size_t n)
{
...
int main()
{
	double A[MATRIX_nXn][MATRIX_nXn];// = { { 1, 2, 3 }, { 4, 5, 6 }, { 7, 8, 9 } };
